...
#include "errorproc.h"
// #include "errorproc.c" // вот без этого не работает, но вообще это не правильно

/*
 * Список вопросов:
 * 1) Зачем мы делаем дублирующие функции Socket и тд и тп - неужели только для поимки багов и вывода ошибок? Peeror хранит посл ошибку
 *
*/
...
int main() {
    // первое, что мы делаем это создаем сокет
    ...
    // putchar(server); // не выводит ничего осмысленного, потому что сокет это некоторая абстракция указывающая на опредленный реусурс системы, чтобы не нужно было хранить больше информации о нем

    // теперь привязываем сокет к адресу для этого нужна функция bind
    struct sockaddr_in adr = {0}; // эта структура использутеся, для того чтобы задать адрес для протокола ipv4
    ...
    Bind(server, (struct sockaddr *) &adr, sizeof adr); // перед &adr мы делаем приведение типов // Функция bind() привязывает уникальное локальное имя к сокету с дескриптором socket
    ...
    ssize_t nread;// Тип данных ssize_t в языке C представляет собой целочисленный тип, который используется для обозначения размера и количества элементов в операциях ввода-вывода и обработки данных. Он определен в заголовочном файле <sys/types.h> и является знаковым целым числом. Основное его отличие от типа size_t заключается в том, что ssize_t может принимать отрицательные значения.
    ...
    sleep(1); // но перед тем как зыкрывать наши сокеты мы подождем одну секунду

    close(fd); // закрываем сокет работы с клиентом
    close(server); //закрываем прослушивающий сокет на стороне сервера
